...
def create_resource_class(
    ...
    try:
        ...
        Logger.error(f"Failed to create ResourceClass {name}: {e}")
        return -1, "", str(e)
...
def delete_resource_class(name: str) -> Tuple[int, str, str]:
    ...
    try:
        custom_api = client.CustomObjectsApi()
        result = custom_api.delete_cluster_custom_object(
            group=DRA_API_GROUP,
            version=get_dra_api_version(),
            plural="resourceclasses",
            name=name,
        ...
        Logger.error(f"Failed to delete ResourceClass {name}: {e}")
        return -1, "", str(e)
...
def create_resource_claim(
    ...
    try:
        ...
        Logger.error(f"Failed to create ResourceClaim {name}: {e}")
        return -1, "", str(e)
...
def get_resource_claim(name: str, namespace: str) -> Optional[Dict]:
    ...
    try:
        ...
        Logger.error(f"Failed to get ResourceClaim {name}: {e}")
        return None
...
def delete_resource_claim(name: str, namespace: str) -> Tuple[int, str, str]:
    ...
    try:
        ...
        Logger.error(f"Failed to delete ResourceClaim {name}: {e}")
        return -1, "", str(e)
...
def verify_dra_driver_crds() -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        Logger.error(f"ResourceClaim API not available: {e}")
    
    # Check DeviceClasses (ResourceClass in older versions)
    ...
    try:
        ...
        Logger.error(f"{device_class_plural.capitalize()} API not available: {e}")
