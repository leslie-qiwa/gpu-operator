...
def verify_dra_uninstallation(
    ...
    start_time = time.time()
    while time.time() - start_time < max_wait_pods:
        running_pods = k8_util.k8_check_pod_terminated(namespace, dra_pods)
        if not running_pods:
            Logger.info("DRA driver pods terminated")
            break
        Logger.info(
            f"Waiting for DRA driver pods to terminate... ({running_pods} still running)"
        )
        time.sleep(5)
    else:
        return False, f"DRA driver pods still running after {max_wait_pods}s"

    # Check ResourceSlices are deleted
    success, err = wait_for_resource_slices_deletion(
        api_version, driver_name, max_retries=max_wait_resources // 5, retry_interval=5
    )
    if not success:
