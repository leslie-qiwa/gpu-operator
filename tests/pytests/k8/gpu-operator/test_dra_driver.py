        ...
        for devcfg in devicecfg_list:
            try:
                ...
                if ret_code == 0 and len(gpu_nodes_list) > 0:
                    ...
                    for node_name in gpu_nodes_list:
...
def test_deviceconfig_dra_driver_mutual_exclusion(
    ...
    for spec_name, tcfg in test_cfg_map.items():
        ...
        if ret_code == 0:
            ...
            # Check which pods are actually running by name patterns
            ...
            device_plugin_running = len(device_plugin_pods) > 0
            dra_driver_running = len(dra_driver_pods) > 0

            Logger.info(
                f"Device plugin pods running: {device_plugin_running} "
                f"(count: {len(device_plugin_pods)})"
            )
            Logger.info(
                f"DRA driver pods running: {dra_driver_running} "
                f"(count: {len(dra_driver_pods)})"
            )

            # Verify mutual exclusion: both should NOT be running simultaneously
            debug_on_failure(
                environment,
                not (device_plugin_running and dra_driver_running),
                f"Mutual exclusion violated: Both device plugin and DRA driver are running. "
                f"Device plugin pods: {len(device_plugin_pods)}, DRA driver pods: {len(dra_driver_pods)}"
            )

            if not device_plugin_running and not dra_driver_running:
                Logger.warning(
                    "Neither device plugin nor DRA driver pods are running. "
                    "Operator may have rejected the configuration."
                )
            elif device_plugin_running:
                Logger.info(
                    "Device plugin is running (DRA driver not running) - mutual exclusion enforced by precedence"
                )
            elif dra_driver_running:
                Logger.info(
                    "DRA driver is running (device plugin not running) - mutual exclusion enforced by precedence"
                )
