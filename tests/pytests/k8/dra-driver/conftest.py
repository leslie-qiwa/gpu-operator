...
def check_dra_api_available(environment):
    ...
    try:
        ...
        device_classes = custom_api.list_cluster_custom_object(
...
def init_dra_testbed(
    ...
    K8Helper.triage(
        environment,
        dra_available,
        f"DRA API is not available in the cluster - {error_msg}",
    )
...
def dra_driver_install(
    ...
    K8Helper.triage(
        environment,
        (major_version > 1) or (major_version == 1 and minor_version >= 32),
    ...
    try:
        ...
        # kubectl equivalent: kubectl get deviceclasses.resource.k8s.io
        device_classes = api_instance.list_cluster_custom_object(
            group="resource.k8s.io", version="v1", plural="deviceclasses"
        )
        for dc in device_classes.get("items", []):
        ...
        Logger.warning(f"Failed to check for orphaned DeviceClass resources: {e}")
